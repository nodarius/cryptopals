#!/usr/bin/python3

from Crypto.Cipher import AES
import base64
import struct
import random

def encrypt_ecb(key, string):
    return AES.new(key, AES.MODE_ECB, 'ignore').encrypt(string)

def decrypt_ecb(key, string):
    return AES.new(key, AES.MODE_ECB, 'ignore').decrypt(string)

def xor_encrypt(str, key):
    full_key = b''
    while(len(full_key) < len(str)):
        full_key += key
    l = [chr(a ^ b) for a, b in zip(str, full_key)]
    return ''.join(l).encode('latin')


def bits(i):
    return struct.pack('L', i)

def encrypt_ctr(key, text, nonce):
    nonce = bits(nonce)
    n = int(len(text) / 16) + 1
    encrypted = b''
    for i in range(0, n):
        block = nonce + bits(i)
        encrypted += encrypt_ecb(key, block)
    res = xor_encrypt(text, encrypted)
    return res

def decrypt_ctr(key, text, nonce):
    return encrypt_ctr(key, text, nonce)


def random_bytes(n):
    l = [chr(random.randint(0, 255)) for i in range(n)]
    l = ''.join(l)
    return l.encode('latin')



plaintext64 = """"""

nonce = 0
def edit(enc, key, offset, newtext):
    plain = decrypt_ctr(key, enc, nonce)
    plain = plain[:offset] + newtext + plain[offset + len(newtext):]
    return encrypt_ctr(key, plain, nonce)

def create_exposed_edit(enc, key):
    return lambda offset, newtext: edit(enc, key, offset, newtext)


def extract_plaintext(enc, myedit):
    enc1 = myedit(0, b'b' * len(enc))
    key = xor_encrypt(enc1, b'b' * len(enc))
    dec = xor_encrypt(enc, key)
    print(dec)

    return dec

def bin_to_str(bin_str):
    res = ''
    for i in range(len(bin_str)):
        res += chr(bin_str[i])
    return res

def unpad(text):
    return text[:-text[len(text) - 1]]

def aes_decrypt(key, string64):
    cipher = AES.new(key, AES.MODE_ECB, "ignore")
    str_bin = base64.b64decode(string64)
    k = cipher.decrypt(str_bin)
    k = bin_to_str(k)
    return unpad(k)



def main():
    key = random_bytes(16)

    plain = base64.b64decode(plaintext64)
    plain = decrypt_ecb(b"YELLOW SUBMARINE", plain)
    enc = encrypt_ctr(key, plain, nonce)
    edit_exposed = create_exposed_edit(enc, key)
    plain = extract_plaintext(enc, edit_exposed)

main()
